...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    ...
    final String databasePath =
        path.join(documentDirectory.path, "sample_db.db");
  ...
  Future<List<NoteModel>> getAllNoteData() async {
    try {
      ...
      final List<Map<String, dynamic>> resultSet =
          await db.query("NOTE", orderBy: "priority");
      return List.generate(resultSet.length, (index) {
  ...
  Future<void> updateNoteItem(NoteModel note) async {
    try {
      final Database db = await database;
      await db.update("NOTE", note.toJson(), where: 'syskey = ${note.syskey}');
    } catch (e) {
      logger.e(e);
    }
  }
